...
int redLEDs [3][3] = {{13,12,11},{10,9,8},{7,6,5}};         //Arrays to store pin numbers for LEDs and input buttons (Left)
int greenLEDs [3][3] = {{4,3,2},{14,15,16},{17,18,19}};     //(right)
int buttons [3][3] = {{31,33,35},{37,39,41},{43,45,47}};
int startButton = 49;
int statusLED [2] = {22,24};                                //22: red, 24: green

int turn = 1;                                               //Counter to keep track of which player's turn it is
int mode = 0;                                               //Variable to store game mode - 0 is ai mode (red), 2 is 2 player (green)
  ...
  if (turn == 1)                                             //If it is player 1's turn to play
  {
    ...
    while (!playerInput)                                     //Loop until player 1 has made their play
    {
      for (int i=0; i<=2 ; i++)                              //Check the state of all buttons to see if the player has chosen a position
        for (int j=0; j<=2; j++)
          if (digitalRead(buttons[i][j]) == LOW)             //If a button is pushed
          {
...
void selectGameMode ()                                //Function to select the game mode, game continues in this mode until reset
{
  ...
  {
    if (digitalRead(buttons[0][1]) == LOW)           //Increment the game mode up or loop back down to mode 0
    {
      if (mode < 1)                                   //改了
        mode++;
      else
    ...
    if (digitalRead(buttons[2][1]) == LOW)           //Increment the game mode down or loop back up to mode 2
    {
      if (mode > 0)
        mode--;
      else
    ...
    if (digitalRead(startButton) == LOW)             //When start game button is pressed, mode is confirmed
      confirmed = true;
  ...
  for (int i=0; i<=2 ; i++)                           //Loop through all 9 board LEDs - i row, j column
  {
    for (int j=0; j<=2; j++)
    {
      if (gameBoard[i][j] == 1)                       //Turn on green LED if 1
        ...
        digitalWrite(redLEDs[i][j],LOW);            //Turn on blue LED if 2
      else
      {
        digitalWrite(greenLEDs[i][j],HIGH);            //Turn off both LEDs if 0
        digitalWrite(redLEDs[i][j],HIGH);
      }
    }
  }
...
void turnOffAll ()                                    //Function to turn off all gameboard LEDs
{
  for (int i=0; i<=2 ; i++)
    for (int j=0; j<=2; j++)
    {
      digitalWrite(greenLEDs[i][j],HIGH);              //Turn off both LEDs
      digitalWrite(redLEDs[i][j],HIGH);
    }
}
...
void flashWin ()                                                                        //Function to flash the wining line
{
  for (int i=0; i<=2 ; i++)                                                             //Loop through all rows and check for a winning line
    if(gameBoard[i][0] == gameBoard[i][1] && gameBoard[i][1] == gameBoard[i][2])
    {
      if (gameBoard[i][0] == 1)
        ...
        for (int k=0; k<=3 ; k++)
        {
          digitalWrite(redLEDs[i][0],HIGH);
          digitalWrite(redLEDs[i][1],HIGH);
          digitalWrite(redLEDs[i][2],HIGH);
          delay (500);
          digitalWrite(redLEDs[i][0],LOW);
          digitalWrite(redLEDs[i][1],LOW);
          digitalWrite(redLEDs[i][2],LOW);
          delay (500);
        }
    }
  ...
  for (int j=0; j<=2; j++)                                                              //Loop through all columns and check for a winning line
    if(gameBoard[0][j] == gameBoard[1][j] && gameBoard[1][j] == gameBoard[2][j])
    {
      if (gameBoard[0][j] == 1)
        ...
        for (int k=0; k<=3 ; k++)
        {
          digitalWrite(redLEDs[0][j],HIGH);
          digitalWrite(redLEDs[1][j],HIGH);
          digitalWrite(redLEDs[2][j],HIGH);
          delay (500);
          digitalWrite(redLEDs[0][j],LOW);
          digitalWrite(redLEDs[1][j],LOW);
          digitalWrite(redLEDs[2][j],LOW);
          delay (500);
        }
    }
  ...
  if(gameBoard[0][0] == gameBoard[1][1] && gameBoard[1][1] == gameBoard[2][2])          //Check first diagonal for a winning line
  {
    ...
    else if (gameBoard[0][0] == 2)
    {
      for (int k=0; k<=3 ; k++)
      {
        digitalWrite(redLEDs[0][0],HIGH);
        digitalWrite(redLEDs[1][1],HIGH);
        digitalWrite(redLEDs[2][2],HIGH);
        delay (500);
        digitalWrite(redLEDs[0][0],LOW);
        digitalWrite(redLEDs[1][1],LOW);
        digitalWrite(redLEDs[2][2],LOW);
        delay (500);
      }
    }
  }
  ...
  if(gameBoard[0][2] == gameBoard[1][1] && gameBoard[1][1] == gameBoard[2][0])          //Check second diagonal for a winning line
  {
    ...
    else if (gameBoard[0][2] == 2)
    {
      for (int k=0; k<=3 ; k++)
      {
        digitalWrite(redLEDs[0][2],HIGH);
        digitalWrite(redLEDs[1][1],HIGH);
        digitalWrite(redLEDs[2][0],HIGH);
        delay (500);
        digitalWrite(redLEDs[0][2],LOW);
        digitalWrite(redLEDs[1][1],LOW);
        digitalWrite(redLEDs[2][0],LOW);
        delay (500);
      }
    }
  }
...
void aITurn ()
{
  ...
  if (firstMove)
  {
    bool played = false;                                       //Check for whether AI has found an unoccupied corner, 先下角落
    int temp = random(4);
    if (temp == 0)
    {
      if (gameBoard[0][0] == 0)
